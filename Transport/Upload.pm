...
class Web::Transport::Upload with (Util::Logger,
...
# Objects
has 'exchange'	=> ( isa => 'Exchange::Main', is  => 'rw', required	=>	0, lazy	=> 1, builder => "setExchange::Main" );
...
method notifyStatus ($data) {
	...
	$self->logDebug("DOING self->newConnection() with data", $data);
	$self->logDebug("self->exchange", $self->exchange());
	
	my $connection = $self->exchange()->newConnection();
