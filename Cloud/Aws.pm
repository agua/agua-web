...
=head2
	...
	PURPOSE
		...
		AMAZON WEB SERVICES (AWS) METHODS FOR Web
...
#### REMOVE aws TABLE ENTRY AND PRINT RESULT
	...
	return if not defined $success;
 	...
 	$self->logError("{ error: 'Web::Cloud::Aws::removeAws    Could not remove user $aws->{username} from user table") and return if not $success;
 	$self->logDebug("{ status: 'Web::Cloud::Aws::removeAws    Removed user $aws->{username} from user table");
	return;
}
...
sub getEc2KeyDir {
	...
	my $username	=	shift;
	#$self->logDebug("username", $username);
	print "Web::Cloud::Aws::getEc2KeyDir    username not defined. Returning\n" and return if not defined $username;
