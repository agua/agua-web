...
method clusterIsBusy ($username, $cluster) {
	$self->logDebug("Web::Cloud::Cluster::clusterIsBusy(username, cluster)");
	$self->logDebug("username", $username);
	$self->logDebug("cluster", $cluster);
...
method setConfigFile ($username, $cluster) {
	...
	my $userdir 	= 	$self->conf()->getKey($self->application(), 'USERDIR');
	my $adminkey 	= 	$self->getAdminKey($username);
	$self->logDebug("adminkey", $adminkey);
...
method updateClusterNodes ($data) {
...
AND cluster='$cluster'
	...
	my $clusterhash = $self->db()->queryhash($query);
 	$self->logDebug("clusterhash", $clusterhash);
	$clusterhash->{minnodes} = $data->{minnodes};
	$clusterhash->{maxnodes} = $data->{maxnodes};
	
	my $success = $self->_removeCluster();	
	return if not defined $success;
 	$self->logError("Could not delete cluster $cluster from cluster table") and return if not $success;

	#### SET TABLE AND REQUIRED FIELDS	
	my $table = "cluster";
	my $required_fields = ["username", "cluster", "minnodes", "maxnodes", "instancetype", "amiid"];
	#### CHECK REQUIRED FIELDS ARE DEFINED
	my $not_defined = $self->db()->notDefined($clusterhash, $required_fields);
    $self->logError("not defined: @$not_defined") and return if @$not_defined;

	#### DO ADD
	$success = $self->_addToTable($table, $clusterhash, $required_fields);	
	$self->logStatus("Could not add cluster $cluster into cluster table") if not $success;
	$self->logStatus("Successful insert of cluster $cluster into cluster table") if $success;
}
...
=cut
	...
	$self->_removeCluster($data);
	
	my $success = $self->_addCluster($data);
	$self->logDebug("_addCluster success", $success);

	$self->notifyStatus($data, "Added cluster $data->{cluster}") and return if $success;
	$self->notifyError($data, "Failed to add cluster $data->{cluster}");
}

method _addCluster ($data) {
	$self->logDebug("data", $data);
...
method removeCluster ($data) {
 	$self->logDebug("data", $data);

	#### REMOVE FROM cluster
	my $success = $self->_removeCluster($data);
	return if not defined $success;
	$self->logError("Could not remove cluster $data->{cluster}") and return if not $success;

	#### REMOVE CELLDIR
	$success = $self->_removeCellDir($data);
	return if not defined $success;
	...
	`rm -fr $configfile`;
	$self->logError("Can't remove cluster $data->{cluster} configfile: $configfile") and return if -f $configfile;

 	$self->logStatus("Removed cluster $data->{cluster}") if $success;
	return;
	
}	#### removeCluster

method _removeCluster ($data) {
	...
	$self->_removeFromTable("clusterworkflow", $data, ["username", "cluster"]);
	
	#### REMOVE FROM clustervars
	$self->_removeFromTable("clustervars", $data, ["username", "cluster"]);
	
	#### REMOVE FROM clusterstatus
	$self->_removeFromTable("clusterstatus", $data, ["username", "cluster"]);
	
	return 1;
}

method _removeCellDir ($data) {
	$self->logDebug("data", $data);
	my $cluster = $data->{cluster};	
	$self->logDebug("cluster", $cluster);
	my $celldir = $self->getCellDir($cluster);
...
method saveClusterWorkflow ($data) {
#### ADD AN ENTRY OR UPDATE AN EXISTING ENTRY IN clusterworkflow TABLE
	...
	if ( not $started ) {
		...
		my $success = $self->_removeClusterWorkflow($data);
		$self->logDebug("_removeClusterWorkflow success", $success);
		
		#### ADD THE NEW CLUSTER TO THE clusterworkflow TABLE
		$success = $self->_addClusterWorkflow($data);
		$self->logDebug("_addClusterWorkflow success", $success);

		$self->notifyError($data, "Could not add workflow $workflow to cluster $data->{cluster}") and return if not $success;

		$self->notifyStatus($data, "Added workflow $workflow to cluster $data->{cluster}");
		return;
	}
	...
	$self->notifyStatus($data, "Successfully removed $oldcluster from clusterworkflow table") and return if defined $oldcluster and not $cluster;
	...
	$success = $self->_addClusterWorkflow($data);
	return if not defined $success;
	$self->logError("Could not add cluster $data->{cluster}") and return if not $success;
...
method _addClusterWorkflow ($data) {
 	$self->logDebug("data", $data);
...
method _removeClusterWorkflow ($data) {
 	$self->logDebug("data", $data);
...
method getMonitor ($clustertype) {
	...
	$clustertype =  $self->conf()->getKey($self->application(), 'CLUSTERTYPE') if not defined $clustertype;
...
method getClusterList ($username, $cluster) {
	...
	#### SET CONFIG FILE
	my $configfile = $self->setConfigFile($username, $cluster);
	my $applications = $self->conf()->getKey($self->application(), "APPLICATIONS");
	my $executable = $self->conf()->getKey($self->application(), "STARCLUSTER");
