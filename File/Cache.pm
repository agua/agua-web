...
=head2
  ...
  PURPOSE
    ...
    FILE CHECKING AND INFO METHODS FOR Web
...
method getFileCaches {
	...
	#### GET DEPTH
	my $depth = $self->conf()->getKey($self->application(), "FILECACHEDEPTH");
...
=cut
    ...
    my $data = $self->data();
    $self->logDebug("data", $data) if defined $data;
	...
	my $fileroot = $self->decideFileroot($username, $owner, $path, $data, $location);
	$self->logDebug("fileroot", $fileroot);
	...
	my $pathdata = $self->fullPathData($fullpath, $outputpath, $outputname, $path);
	$self->logDebug("pathdata", $pathdata);
	
	$self->notifyStatus($pathdata, "ok")

}

method fullPathData ($fullpath, $outputpath, $outputname) {
	$self->logDebug("fullpath", $fullpath);
	$self->logDebug("outputpath", $outputpath);
	$self->logDebug("outputname", $outputname);
	...
	if ( -d $fullpath ) {
        ...
        foreach my $file (@filenames)
        {
			...
			$self->logDebug("Doing self->fileData()");
            push @{$data->{items}},  $self->fileData($outputname, $filepath, $outputpath);
        }
    }
    elsif ( -f $fullpath )
    {
        ...
        $data = $self->fileData($outputname, $fullpath, $parentDir);
    }
	else
	{
...
=cut
    ...
    $self->logDebug("fileStats", $fileStats);
	...
	$self->logDebug("-f filepath", -f $filepath);
	$self->logDebug("-z filepath", -z $filepath);
    ...
    if ( $fileStats->{directory} =~ /^true$/ ) {
        ...
        my $sample = $self->sampleFile($filepath, $bytes);
...
method fileData ($path, $filepath, $parentDir) {
=head2
...
=cut
    ...
    $data->{name}		=	$name;
    $data->{path}		=	"$parentDir/$name";
    $data->{parentPath}	=	$parentDir;
    $data->{parentDir}	=	$parentDir;
    $data->{directory}	=	$fileStats->{directory};
    $data->{size}		=	$fileStats->{filesize};
    $data->{modified}	=	$fileStats->{modified};
...
method sampleFile ($filepath, $bytes) {
	...
	if ( -B $filepath
		...
		open FILEHANDLE, "<", $filepath or $self->logDebug("Can't open filepath", $filepath);
		seek(FILEHANDLE, 0, 0);
		read(FILEHANDLE, $sample, $bytes);
	}
	$self->logDebug("RAW sample", $sample);
	
	#### SET TO '' IF FILE IS EMPTY
	$sample = '' if not defined $sample;
	...
	#### SET DEFAULT IF UNICODE IN STRING
	$sample = "Non-ASCII characters in file" if not $self->canJson($sample);

	return $sample;
}

method canJson ($string) {
	$string = qq{{"string":"$string"}};
	$self->logDebug("string", $string);

	my $jsonparser = $self->jsonparser();
	eval {
		$self->logDebug("BEFORE json decode");
		$jsonparser->decode($string);
		#$jsonparser->allow_nonref->utf8->relaxed->escape_slash->loose->allow_singlequote->allow_barekey->decode($sample);
		$self->logDebug("AFTER json decode");
...
=cut
	...
	return "{ error: 'Web::File::Cache::filePeek    offset not defined' }" if not defined $offset;
	return "{ error: 'Web::File::Cache::filePeek    filepath not defined' }" if not defined $filepath;
	return "{ error: 'Web::File::Cache::filePeek    lines and bytes not defined' }" if not defined $lines and not defined $bytes;

	#### EXIT IF FILEPATH NOT FOUND
	if ( not -f $filepath and not -d $filepath )
	{
		return "{  error: 'Web::File::Cache::filepeek    file or directory not found: $filepath' }";
	}
	...
	if ( defined $lines and defined $offset )
	{
		...
		{
			return "{  error: 'Web::File::Cache::filepeek    peek at offset $offset and lines $lines not defined for filepath: $filepath' }";
		}		
		$peek =~ s/\n/\\n/g;
		$peek =~ s/'/\\'/;
	...
	elsif ( defined $bytes and defined $offset )
	{
		...
		if ( not defined $peek or not $peek )
		{
			return "{  error: 'Web::File::Cache::filepeek    peek at offset $offset and bytes $bytes not defined for filepath: $filepath' }";
		}
...
=cut
	...
	if ( not defined $json->{requestor} ) {
		...
		return {error => 'Web::File::Cache::_checkFile    fileroot not defined for user: $username and filepath: $filepath' } if not defined $fileroot or not $fileroot;
	...
	my $fileroot = $self->getFileroot($username);
	return { error => 'Web::File::Cache::_checkFile    Fileroot not found for user' } if not defined $fileroot;
