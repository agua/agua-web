...
=head2
	...
	PURPOSE
		...
		AUTHENTICATION AND ACCESS PRIVILEGE METHODS FOR Web
...
=cut
	...
	#### CHECK SESSION ID
	$self->logError("sessionid not defined") and exit if not defined $sessionid;
	
	#### VALIDATE TEST USER
	my $guestuser 	= $self->conf()->getKey($self->application(), "GUESTUSER");
	my $testuser	=	$self->conf()->getKey("database", "TESTUSER");
	$self->logDebug("guestuser", $guestuser);	
	$self->logDebug("testuser", $testuser);

	if ( $username eq $guestuser ) {
		return 0 if not $self->validateGuest();
		$self->validated(1);
		return 1;
	}
	elsif ( $username eq $testuser ) {
		return 0 if not $self->validateTest();
		$self->validated(1);
		return 1;
	}
	else {
		...
		return 0 if not defined $validated;
...
return 1;
	...
	my $guestaccess = $self->conf()->getKey($self->application(), "GUESTACCESS");
	$self->logDebug("guestaccess", $guestaccess);
	my $guestlock = $self->conf()->getKey($self->application(), "GUESTLOCK");
