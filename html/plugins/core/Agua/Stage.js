/* STAGE METHODS INHERITED BY Agua.js */

console.log("Loading plugins.core.Agua.Stage");
...
// UPDATE STAGE status, started AND completed VALUES IN Agua.data.stages
	console.log("Agua.Stage.updateStagesStatus    stageList: ");
	console.dir({stageList:stageList});
	...
	console.log("Agua.Stage.updateStagesStatus     projectName: " + projectName);
	console.log("Agua.Stage.updateStagesStatus     workflowName: " + workflowName);
	
	for ( var i = 0; i < stages.length; i++ ) {
		console.log("Agua.Stage.updateStagesStatus     stage " + i + " : ");
		console.dir({stage:stages[i]});

		// ONLY DISPLAY STAGES FOR THIS PROJECT
		if ( stages[i].project != projectName )	continue;
		if ( stages[i].workflow != workflowName )	continue;

		for ( var k = 0; k < stageList.stages.length; k++ ) {
			console.log("Agua.Stage.updateStagesStatus     stageList.stages " + k);
			if ( stages[i].number == stageList.stages[k].number ) {
				stages[i].status = stageList.stages[k].status;
				stages[i].started = stageList.stages[k].started;
				stages[i].completed = stageList.stages[k].completed;
				continue;
			}
		}
	}
	
	console.log("Agua.Stage.updateStagesStatus    END");		
},	
...
// ALSO UPDATE THE REMOTE DATABASE.
	...
	this.sendRequest({
		data		: 	stageObject,
		mode		:	"updateStageSubmit",
		module 		: 	"Agua::Workflow",
		sourceid	:	this.id,
		callback	:	"handleUpdateStageSubmit"
	});

	return 1;
},
handleUpdateStageSubmit : function (response) {
	console.log("Agua.Stage.handleUpdateStageSubmit    response: " + dojo.toJson(response));
