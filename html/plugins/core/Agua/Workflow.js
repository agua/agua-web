/* WORKFLOW METHODS INHERITED BY Agua.js */

console.log("Loading plugins.core.Agua.Workflow");
...
// RETURN WORKFLOW IN workflows IDENTIFIED BY PROJECT AND WORKFLOW NAMES
	...
	if ( ! this.isProject(workflowObject.project) ) {
		console.log("Agua.Workflow._getWorkflow     No project '" + workflowObject.project + "' found in workflowObject. Returning false.");
		return false;
	}
...
// ADD AN EMPTY NEW WORKFLOW OBJECT TO A PROJECT OBJECT
	...
	data.number = number;
	console.log("Agua.Workflow.addWorkflow    data: " + dojo.toJson(data, true));
	...
	this.sendRequest({
		...
		sourceid	:	this.id,
		callback	:	"_addWorkflow"
	});
},
_addWorkflow : function(response) {
	console.log("Agua.Cluster.handleAddCluster   response: ", response);
	var data	=	response.data;
	var keys = ["project", "name", "number"];
	var added = this.addData("workflows", data, keys);
},
...
_removeWorkflow : function(response) {
	...
	this.removeObjectsFromData("stages", data, keys);
	this.removeObjectsFromData("stageparameters", data, keys);
	
	// RENUMBER WORKFLOWS
	this.renumberWorkflows(data.project);
