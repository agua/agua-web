...
startup : function () {
	...
	console.log("Agua.startup    DOING this.loadPlugins()");
	this.loadPlugins();

	////// SET EXCHANGE 
	var thisObject = this;
	console.log("%cAgua.startup    BEFORE this.setExchange()", "color: blue");
	
	console.log("%cAgua.startup    Exchange:", Exchange);
	...
	this.setExchange(Exchange).then( function() {
		...
		console.log("Agua.startup    DEBUG: DOING thisObject.loadPlugins()");
		thisObject.loadPlugins();
	});

	console.log("%cAgua.startup    AFTER this.setExchange()", "color: blue");
...
startPlugins : function () {
	...
	return this.initialisePlugins(this.pluginsList);
},
...
initialisePlugins : function (pluginsList, pluginsArgs) {
	console.log("Agua.initialisePlugins    pluginsList: " + dojo.toJson(pluginsList));
	console.log("Agua.initialisePlugins    pluginsArgs: " + dojo.toJson(pluginsArgs));
	//console.log("Agua.initialisePlugins    this: ");
	//console.dir({this:this});

	if ( ! this.controllers ) return;
	//console.log("Agua.initialisePlugins    this.controllers:");
	//console.dir({this_controllers:this.controllers});
	
	for ( var className in this.controllers ) {
		if ( className == "data") continue;
		
		console.log("Agua.initialisePlugins    className: " + className);
		
		var controller = this.controllers[className];
		console.log("Agua.initialisePlugins    controller: ");
		console.dir({controller:controller});
		
		console.log("Agua.initialisePlugins    DOING " + className + ".startup()");
		controller.startup();
	}
...
doLogout : function () {
	...
	for ( var i = 0; i < buttons.length; i++ ) {
		...
		var name = controller.id.match(/[^_]+_([^_]+)/)[1]; 
...
// UTILS
getComponent : function(string) {
	var array = string.split("\.");
	console.log("Agua.getComponent    string: " + string);
	console.log("Agua.getComponent    array: ");
	console.dir({array:array});

	var controller = Agua.controllers[array[0]];
	console.log("Agua.getComponent    controller: " + controller);
	var widget = controller.tabPanes[0];
	console.log("Agua.getComponent    widget: " + widget);
	console.dir({widget:widget});
	if (array.length == 1) {
		return widget;
	}
	
	var component = widget.core[array[1]];
	console.log("Agua.getComponent    component: " + component);
