...
loadDragItems : function (itemArray) {
	...
	for ( var k = 0; k < allNodes.length; k++ ) {
		...
		require([rowClass], function (Module) {
			var plugin = new Module(itemObject);
			//console.log("DnDSource.loadDragItems    INSIDE plugin: " + plugin);
			//console.dir({plugin:plugin});

			thisObject.childWidgets.push(plugin);
			node.innerHTML = '';
			node.appendChild(plugin.domNode);
		});
