console.log("Loading plugins/workflow/runstatus/StageStatus");

define( "plugins/workflow/RunStatus/StageStatus",
[
	"dojo/_base/declare",
	"dojo/_base/lang",
	"dijit/_Widget",
...
return declare("plugins/workflow/RunStatus/StageStatus",
	[ _Widget, _TemplatedMixin, _WidgetsInTemplateMixin, Common ], {

// templateString : String
//		The template of this widget. 
templateString: dojo.cache("plugins", "workflow/RunStatus/templates/stagestatus.html"),
...
displayStagesStatus : function (stages) {
	...
	for ( var i = 0; i < stages.length; i++ ) {
		console.log("StageStatus.displayStagesStatus     Doing stages[" + i + "]: ");
		...
		//console.log("StageStatus.displayStagesStatus     stages[i].duration: " + stages[i].duration);

		stages[i].lapsed = this.calculateLapsed(stages[i]);
		//console.log("StageStatus.displayStagesStatus     stages[i].lapsed: " + stages[i].lapsed);
		...
		var stagestatusRow = new StageStatusRow(stages[i]);
		
		console.log("StageStatus.displayStagesStatus     stagestatusRow:");
...
calculateDuration : function (stage) {
	console.log("StageStatus.calculateDuration    stage.completed: " + stage.completed);
		...
		&& stage.started != "0000-00-00 00:00:00" )
	{
		...
		var currentDate;
		if ( stage.completed == "0000-00-00 00:00:00" ) {
			duration = "";
			console.log("StageStatus.calculateDuration    Returning duration: " + duration);
			return duration;
		}
		if ( ! stage.completed 
