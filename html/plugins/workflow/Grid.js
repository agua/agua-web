...
templateString: dojo.cache("plugins", "workflow/templates/grid.html"),
...
setWorkflows : function (projectName) {
	...
	console.log("Grid.setWorkflows    AFTER this.clear");
	...
	dojo.forEach(workflows, function(workflow, i) {
		...
		var gridWorkflow = new GridWorkflow({
			title: workflowName,
			project: projectName,
		...
		thisObject.addToGrid(gridWorkflow);
	});	
},
addToGrid : function (gridWorkflow) {
	console.log("Grid.addToGrid    this: " + this);
	console.dir({this:this});
	console.log("Grid.addToGrid    gridWorkflow:");
	console.dir({gridWorkflow:gridWorkflow});
	//console.log("Grid.addToGrid    this.gridContainer:");
	//console.dir({this_gridContainer:this.gridContainer});

	//this.gridContainer.containerNode.appendChild(gridWorkflow.titlePane.domNode);
	console.log("Grid.addToGrid    BEFORE this.gridContainer.addChild(gridWorkflow)");
	this.gridContainer.addChild(gridWorkflow.titlePane);
	console.log("Grid.addToGrid    AFTER this.gridContainer.addChild(gridWorkflow)");

	// PUSH ONTO ARRAY OF CHILD WIDGETS
	this.childWidgets.push(gridWorkflow);

	//console.log("Grid.addToGrid    END");
},
...
// RESET number IN ALL projectWorkflows AND THEIR workflowStages
	...
	var workflows = this.gridContainer.getChildren();
...
setSubscriptions : function () {
	...
	connect.subscribe("/dojox/mdnd/adapter/dndToDojo/over", null,  function(arg) {
		console.log("dndToDojo/over");
	});
	
	connect.subscribe("/dojox/mdnd/adapter/dndToDojo/out", null, function(arg) {
		console.log("dndToDojo/out");
	});
	connect.subscribe("/dojox/mdnd/adapter/dndToDojo/drop", null, function(arg) {
		console.log("dndToDojo/drop");
	});
	connect.subscribe("/dojox/mdnd/adapter/dndToDojo/cancel", null, function(arg) {
		console.log("dndToDojo/cancel");
	});
	
	connect.subscribe("/dojo/dnd/manager/overSource", function(source){
		console.debug("/dojo/dnd/manager/overSource", source);
	});
	
	connect.subscribe("/dojox/mdnd/drag/start", function(node, targetArea, indexChild){
		console.dir({node: node});
	...
	connect.subscribe("/dojox/mdnd/drop", null, function(node, targetArea, indexChild){
	...
	connect.subscribe("/dojox/mdnd/dropMode.OverDropMode/getDragPoint", function(node, targetArea, indexChild){
	...
	/*
	//connect.subscribe("/dojox/mdnd/drag/over", function(source){
	//	console.debug("over", source);
	//});
	
	//connect.subscribe("/dojox/mdnd/drag/cancel", function(source, nodes, copy, target) {
	//	console.debug("cancel", source);
	//});
	
	//connect.subscribe("/dojox/mdnd/out", function(source){
	//	console.debug("out", source);
	//});
	//connect.subscribe("/dojox/mdnd/cancel", function(source){
	//	console.debug("cancel", source);
	//});
	*/
}
