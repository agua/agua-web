/* DISPLAY WORKFLOW TARGET, SHOW STAGES IN A WORKFLOW, ALLOW USER TO MANAGE PROJECTS AND WORKFLOWS
...
console.log("%cplugins/workflow/Workflows    LOADING", "color: blue; font-size: 12px");
...
cssFiles : [
	require.toUrl("plugins/workflow/css/workflows.css")
],
...
constructor : function (args) {
	console.log("Workflows.constructor     caller: " + this.constructor.caller.nom);			
	console.log("Workflows.constructor     args:");			
	console.dir({args:args});
	
	// SET ID
	this.id = dijit.getUniqueId("plugins_workflow_Workflows");
	console.log("Workflows.constructor     this.id: ", this.id);			
	...
	console.log("Workflows.constructor     attachPoint: " + this.attachPoint);			
...
startup : function () {
	...
	console.log("Workflows.startup    this.id: " + this.id);
	...
	console.log("Workflows.startup    DOING this.attachPane(), attachPoint: " + this.attachPoint);		
	this.attachPane();

	// SET INPUT/OUTPUT CHAINER
	console.log("Workflows.startup    DOING this.setWorkflowIO()");
	this.setWorkflowIO();
	
	// SET SELECTIVE DIALOG FOR copyWorkflow	
	console.log("Workflows.startup    DOING this.setSelectiveDialog()");
	this.setSelectiveDialog();
	
	// SET INTERACTIVE DIALOG FOR copyProject
	console.log("Workflows.startup    DOING this.setInteractiveDialog()");
	...
	// CREATE DROP TARGET
	console.log("Workflows.startup    DOING this.setDropTarget()");
	this.setDropTarget();

	// START CASCADE OF COMBO LOADING:
	// PROJECT COMBO, WORKFLOW COMBO, DROP TARGET
	console.log("Workflows.startup    DOING this.setProjectCombo()");
	this.setProjectCombo();
	
	// SET ONCLICK FOR PROJECT AND WORKFLOW BUTTONS
	console.log("Workflows.startup    DOING this.setComboButtons()");
	this.setComboButtons();

	// SET LISTENERS
	console.log("Workflows.startup    DOING this.setProjectListeners()");
	this.setProjectListeners();
	this.setWorkflowListeners();

	// SET SUBSCRIPTIONS
	console.log("Workflows.startup    DOING this.setSubscriptions()");
...
setSubscriptions : function () {
	console.log("Workflows.setSubscriptions    this.setSubscriptions()");
	if ( ! Agua.updater )	return;
	console.log("Workflows.setSubscriptions    Agua.updater:");
	console.dir({Agua_updater:Agua.updater});
...
// GENERATE CONTEXT MENU
	...
	this.contextMenu = new StageMenu(
		{
			parentWidget: this,
			core: this.core
		}
...
// CREATE DROP TARGET
	console.log("Workflows.setDropTarget    this.dropTargetContainer: " + this.dropTargetContainer);
	console.log("Workflows.setDropTarget    this.contextMenu: " + this.contextMenu);
	...
	console.log("Workflows.setDropTarget    this.dropTarget: " + this.dropTarget);
	console.dir({this_dropTarget:this.dropTarget});
	
	if ( this.dropTarget == null ) {
		console.log("Workflows.setDropTarget    this.dropTarget is null. Returning");
		return;
	}
},
...
// THE STAGE INFORMATION INCLUDING number
	...
	if ( this.dropTarget == null )
	{
		this.dropTarget = new Target( this.dropTargetContainer,
			{
				accept: ["draggableItem"],
				contextMenu : this.contextMenu,
				parentWidget : this,
...
_updateDropTarget : function (stages) {
	...
	if ( allNodes.length != 0 && this.shared ) {
		...
		console.log("Workflows._updateDropTarget    Shared. Doing this.loadParametersPane(node_0.application)");
		this.loadParametersPane(allNodes[0].application);
		this.getStatus(allNodes[0].application);
...
_updateDropTargetNodes : function(stages) {
	...
	console.log("Workflows._updateDropTargetNodes    BEFORE insertNodes this.dropTarget: " + this.dropTarget);
	console.dir({this_dropTarget:this.dropTarget});
	
	this.dropTarget.insertNodes(false, dataArray);

	// SET NODE CHARACTERISTICS - ONCLICK, CLASS, ETC.
	console.log("Workflows._updateDropTargetNodes    AFTER insertNodes this.dropTarget: " + this.dropTarget);
...
_updateDropTargetStageRows : function (stages, allNodes) {
	...
	dojo.forEach(allNodes, function (node, i) {
		...
		// INSTANTIATE ROW 
		var args 			= dojo.clone(stages[i]);
		args.core 			=	thisObject.core;
		args.parentWidget 	= 	thisObject;
		var stageRow = new StageRow(args);
		...
		dojo.connect(node, "onclick",  dojo.hitch(thisObject, function(event)
			{
				console.log("Workflows._updateDropTargetStageRows    XXXXXXXXXXXXXXX ONCLICK call to this.loadParametersPane()");
				...
				this.loadParametersPane(node.application);
			}
		));
...
*/
	...
	setTimeout(function () {
		...
		thisObject.loadParametersPane(allNodes[0].application);
...
//	RETRIEVE MIN/MAX NODES FOR THE CLUSTER
	...
	if ( clusterName == null || ! clusterName ) {
		console.log("Workflows.setClusterNodes    clusterName NULL OR EMPTY");
	
		this.minNodes.set('value', 0);	
		this.maxNodes.set('value', 0);	
		console.log("Workflows.setClusterNodes    RETURNING");

		return;
	}
	console.log("Workflows.setClusterNodes    clusterName NOT NULL OR EMPTY");
...
updateClusterNodes : function () {
	...
	console.log("Workflows.updateClusterNodes    clusterObject: ", clusterObject);
	...
	console.log("Workflows.updateClusterNodes    BEFORE Agua.updateClusterNodes(clusterObject)");
	Agua.updateClusterNodes(clusterObject);
	console.log("Workflows.updateClusterNodes    AFTER Agua.updateClusterNodes(clusterObject)");
...
saveClusterWorkflow : function (event) {
	console.log("Workflows.saveClusterWorkflow    this.id: ", this.id);
	...
	clusterObject.username 	= 	username;
	...
	if ( Agua.isClusterWorkflow(clusterObject) ) {
		console.log("Workflows.saveClusterWorkflow    Skipping as clusterObject already exists", clusterObject);
		//this.savingCluster = false;
		return;
	}
	
	Agua.saveClusterWorkflow(clusterObject, this.id);
	
	//this.savingCluster = false;
},
handleSaveClusterWorkflow : function (response) {
	console.log("Workflows.handleSaveClusterWorkflow   response: ", response);
	var data	=	response.data;
	console.log("Workflows.handleSaveClusterWorkflow   data: ", data);

	if ( data.error ) {
		console.log("Workflows.handleSaveClusterWorkflow   data.error: " + data.error);
		this.clusterCombo.value = "";
	}
	else {
		Agua._removeClusterWorkflow(data);
		Agua._addClusterWorkflow(data);	

		this.setClusterNodes(data.cluster);
	}
...
loadParametersPane : function (stageObject) {
...
// OVERLOAD THIS TO PASS ADDITIONAL ARGUMENTS TO Parameters.load()
	...
	console.log("Workflows.loadParametersPane    caller: " + this.loadParametersPane.caller.nom);
	console.log("Workflows.loadParametersPane    stageObject: ");
	console.dir({stageObject:stageObject});
	
	// WARN AND QUIT IF NO NODE PASSED, E.G., IF WORKFLOW HAS NO STAGES
	if ( stageObject == null ) {
		//console.log("Workflows.loadParametersPane     Passed stageObject is null (no applications in dropTarget). Returning.");
	...
	if ( this.core.parameters != null ) {
		console.log("Workflows.loadParametersPane    Doing this.core.parameters.load(stageObject)");
		this.core.parameters.load(stageObject);
	}
	else {
...
getWorkflow : function () {
	...
	//console.log("Workflows.getWorkflow     this:");
	//console.dir({this:this});
	
	return this.workflowCombo.get('value');
},
getWorkflowNumber : function () {
	console.log("Workflows.getWorkflowNumber()");
	var project = this.getProject();
	var workflow = this.getWorkflow();
...
indexOfRunningStage : function () {
	console.log("Workflows.indexOfRunningStage    ");
...
setInteractiveDialog : function () {
	...
	this.interactiveDialog = new InteractiveDialog(
		{
			title 				:	title,
			message 			:	message,
			inputMessage 		:	inputMessage,
...
console.log("%cplugins/workflow/Workflows    COMPLETED", "color: blue;");
