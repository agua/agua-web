...
=head2
	...
	PURPOSE
		...
		2. USER ADDS/REMOVES TRACKS TO/FROM VIEW
			...
			2.1 ADD/REMOVE FEATURE trackData.json INFORMATION TO/FROM data/trackList.json
...
class Web::View::JBrowse with (Engine::Cluster::Checker,
	Engine::Cluster::Jobs,
	Engine::Cluster::Loop,
	Engine::Cluster::Usage,
	Engine::Common::SGE,
	Table::Main,
	Util::Logger,
	Util
...
=head2
	...
	PURPOSE	
		...
		2. CREATE refSeqs.json JSON FILE CONTAINING ENTRIES FOR ALL CHROMOSOMES
    ...
    OUTPUT
        ...
        1. OUTPUT FILE refSeqs.json IN OUTPUT DIRECTORY
...
=cut
	...
	#### CHECK INPUTS
	$self->logError("Web::View::JBrowse::generateRefseq    outputdir not defined") and exit if not defined $outputdir;
	$self->logError("Web::View::JBrowse::generateRefseq    inputdir not defined") and exit if not defined $inputdir;
	$self->logError("Web::View::JBrowse::generateRefseq    chunksize not defined") and exit if not defined $chunksize;
...
--chunksize $chunksize};
	...
	#### REPORT 
	my $refseqfile = "$outputdir/data/seq/refSeqs.json";
...
=head2
	...
	PURPOSE	
		...
		CREATE A refSeqs.json JSON FILE CONTAINING ENTRIES FOR ALL CHROMOSOMES
...
=cut
	...
	#### CHECK INPUTS
	$self->logError("Web::View::JBrowse::generateRefseq    outputdir not defined") and exit if not defined $outputdir;
	$self->logError("Web::View::JBrowse::generateRefseq    chromofile not defined") and exit if not defined $chromofile;
	$self->logError("Web::View::JBrowse::generateRefseq    chunksize not defined") and exit if not defined $chunksize;
	...
	#### OPEN OUTPUT FILE
	my $outputfile = "$outputdir/refSeqs.json";
...
=head2
	...
	PURPOSE
		...
		ALL FEATURES IN THE trackList.json FILE
...
method addTrack (Str $trackdatafile, Str $trackinfofile) {
	...
	$self->writeJson($trackinfofile, $trackinfo, "trackList = ");
...
method addTrackinfo (Str $sourceinfofile, Str $targetinfofile) {
	...
	$self->writeJson($targetinfofile, $targetinfo, "trackList = ");	
...
method getTrackinfo ($trackinfofile) {
	...
	my $trackinfo = $self->readJson($trackinfofile, "trackList = ");
...
=head2
	...
	NOTES
		...
		2. COPY refSeqs.json TO EACH SUB DIRECTORY
...
=cut
	...
	#### 1. CREATE DIRECTORIES FOR OUTPUT FILES
	####  	AND COPY refSeqs.json TO EACH SUB DIRECTORY
...
=cut
	...
	#### 3. COPY refSeqs.json FILE TO THE data DIRECTORY
	my $inputrefseq = "$datadir/refSeqs.json";
...
=head2
	...
	PURPOSE
		...
		PARSE REFERENCE SEQUENCE NAMES FROM refSeqs.json JSON FILE
